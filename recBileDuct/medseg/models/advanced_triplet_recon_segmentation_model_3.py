...
from medseg.common_utils.basic_operations import construct_input, set_grad, rescale_intensity, my_construct_input
from medseg.common_utils.save import save_testing_images_results

from medseg.models.ebm.TAGEncoder import TAGEncoder, TAGDecoder


class AdvancedTripletReconSegmentationModel_3(nn.Module):
    def __init__(self, network_type='FCN_16_standard', image_ch=1,
                 learning_rate=1e-4,
                 encoder_dropout=None,
                 decoder_dropout=None,
                 num_classes=2, n_iter=1,
        ...
        """
        super(AdvancedTripletReconSegmentationModel_3, self).__init__()
    ...
    def get_network(self, checkpoint_dir=None):
        ...
        if network_type in ['FCN_16_standard', 'FCN_16_standard_w_o_filter', 'FCN_16_standard_share_code', 'TAGNet']:
            ...
            if 'FCN' in network_type:
                ...
                print("load TAGNet encoder and decoder...")
                image_encoder = TAGEncoder(input_channel=1, z_level_1_channel=512 // reduce_factor, z_level_2_channel=512 //
                                                    reduce_factor, feature_reduce=reduce_factor, if_SN=False, encoder_dropout=self.encoder_dropout,
                                                    norm=nn.BatchNorm2d, res_conn=True)
                segmentation_decoder = TAGDecoder(input_channel=512//reduce_factor, feature_scale=reduce_factor, output_channel=self.num_classes)
            ...
            if self.use_gpu:
                if torch.cuda.device_count() > 1:
                    ...
                    segmentation_decoder = nn.DataParallel(segmentation_decoder)
                    shape_decoder = nn.DataParallel(shape_decoder)
                    shape_encoder = nn.DataParallel(shape_encoder)
                    image_decoder = nn.DataParallel(image_decoder)
    ...
    def encode_shape(self, segmentation, is_label_map=False, disable_track_bn_stats=False, temperature=2):
        ...
        新的改动，进入细化网络的通道数为1，这样对于非label进行sigmoid，label不做处理 （改动无效）
        '''
        ...
        # print('is label? - ', is_label_map, segmentation.shape)
        # prediction_map = my_construct_input(segmentation, is_labelmap=is_label_map, use_gpu=self.use_gpu)
        if disable_track_bn_stats:
    ...
    def perturb_latent_code(self, latent_code, decoder_function, decoder_name='image_decoder', label_y=None,
        ...
        if perturb_type == 'dropout':
            ...
            if perturb_type == 'spatial':
                masked_latent_code, mask = mask_latent_code_spatial_wise(latent_code, num_classes=self.num_classes, decoder_function=decoder_function, features=self.features, use_features=False if decoder_name=='image_decoder' else True,
                ...
                masked_latent_code, mask = mask_latent_code_channel_wise(latent_code, num_classes=self.num_classes, decoder_function=decoder_function, features=self.features, use_features=False if decoder_name=='image_decoder' else True,
                                                                         label=label_y, percentile=threshold, random=random_threshold, loss_type=loss_type, if_detach=if_detach, if_soft=if_soft)
            else:
                raise NotImplementedError
        if if_detach:
    ...
    # TODO 辨别分割分支和重建分支
    def decoder_inference(self, decoder, decoder_name, latent_code, eval=False, disable_track_bn_stats=False):
        decoder_state = decoder.training
        if eval:
            decoder.eval()
            with torch.no_grad():
                if decoder_name == 'segmentation_decoder':
                    logit = decoder(latent_code, self.features)
                else:
    ...
    def standard_training(self, clean_image_l, label_l, perturbed_image, separate_training=False, compute_gt_recon=True, update_latent=True, disable_track_bn_stats=False):
        ...
        standard_supervised_loss = basic_loss_fn(
            pred=y_0, target=label_l.detach(), loss_type= 'foreground dice') # 'cross entropy'
        # print("standard_supervised_loss (foreground dice loss): ", standard_supervised_loss)

        # 重建分支的损失
        image_recon = self.decode_image(z_i)
        assert image_recon.shape == clean_image_l.shape, 'image_recon shape != clean_image shape'

        image_recon_loss = 0.7 * \
            torch.nn.MSELoss(reduction='mean')(
                input=image_recon, target=clean_image_l)
        
        # image_recon_loss = 0.7 * basic_loss_fn(pred=image_recon, target=clean_image_l, loss_type='ssim')
        # image_recon_loss = torch.from_numpy(np.array(0.))
        # 细化网络
        if compute_gt_recon:
    ...
    def hard_example_generation(self,
        ...
        if gen_corrupted_image:
            self.reset_all_optimizers()
            perturbed_z_i_0, img_code_mask = self.perturb_latent_code(latent_code=self.z_i, decoder_name='image_decoder',
            ...
            perturbed_image_0 = self.decoder_inference(decoder=self.model['image_decoder'], decoder_name='image_decoder',
        ...
        if gen_corrupted_seg:
            ...
            perturbed_z_0, shape_code_mask = self.perturb_latent_code(latent_code=self.z_s, decoder_name='segmentation_decoder',
                                                                      label_y=label_l,
            ...
            perturbed_y_0 = self.decoder_inference(decoder=self.model['segmentation_decoder'], decoder_name='segmentation_decoder',
    ...
    def fast_predict(self, input, disable_track_bn_stats=False):
        ...
        if not self.training:
            with torch.no_grad():
                if self.network_type == 'TAGNet':
                    z_i, z_s, self.features = encoder(input)
                else:
    ...
    def evaluate(self, input, targets_npy, n_iter=None, pid=None):
        '''
        ...
        # gc.collect()  # collect garbage
        self.train(if_testing=True)
        pred = self.predict(input, n_iter=n_iter) # ([b, 2, 256, 256])
        if self.num_classes == 1:
            pred_npy = pred[:,0,...].cpu().numpy()
        else:
            pred_npy = pred.max(1)[1].cpu().numpy() # (b, 256, 256)

        self.running_metric.update(
            label_trues=targets_npy, label_preds=pred_npy, pid=pid)
        
        if input.shape[1] == 3:
            self.cur_eval_images = input.data.cpu().numpy()[:, 1, :, :]
        else:
    ...
    def save_model(self, save_dir, epoch_iter, model_prefix=None, save_optimizers=False):
        ...
        for model_name, model in self.model.items():
            # multi-GPUs saved model, by sophia.
            if torch.cuda.device_count() > 1:
...
if __name__ == '__main__':
    ...
    solver = AdvancedTripletReconSegmentationModel_3(image_ch=3,
        network_type='TAGNet', num_classes=2, n_iter=2, use_gpu=False)
    model = solver.model
    images = torch.randn(8, 3, 256, 256)
    pred = solver.evaluate(images, targets_npy=torch.rand([8,256,256]), n_iter=2)
    print ('output shape: ',pred.size())
