...
def mask_latent_code_channel_wise(latent_code, decoder_function, features, use_features, label, num_classes=2, percentile=1 / 3.0, random=False, loss_type='corr', if_detach=True, if_soft=False):
    """
    ...
    if loss_type == 'corr':
        ...
        loss = torch.mean(decoder_function(code) * gt_y) if use_features == False else torch.mean(decoder_function(code, features) * gt_y)
    elif loss_type == 'mse':
        loss = torch.mean((decoder_function(code) - gt_y)**2) if use_features == False else torch.mean((decoder_function(code, features) - gt_y)**2)
    elif loss_type == 'ce':
        logit = decoder_function(code) if use_features == False else decoder_function(code, features)
...
def mask_latent_code_spatial_wise(latent_code, decoder_function, features, use_features, label, num_classes, percentile=1 / 3.0, random=False, loss_type='corr', if_detach=True, if_soft=False):
    '''
