...
class AdvancedTripletReconSegmentationModel_2(nn.Module):
    def __init__(self, network_type='FCN_16_standard', image_ch=1,
        ...
        """
        super(AdvancedTripletReconSegmentationModel_2, self).__init__()
    ...
    def get_network(self, checkpoint_dir=None):
        ...
        if network_type in ['FCN_16_standard', 'FCN_16_standard_w_o_filter', 'FCN_16_standard_share_code', 'TAGNet']:
            ...
            if 'FCN' in network_type:
                ...
                image_decoder = TAGDecoder(input_channel=512//reduce_factor, feature_scale=reduce_factor, output_channel=self.image_ch, last_layer_act=nn.Sigmoid())
        ...
        if disable_track_bn_stats:
            with _disable_tracking_bn_stats(image_decoder):
                if self.network_type == 'TAGNet':
                    pred = image_decoder(latent_code, self.features)
                else:
    ...
    def encode_shape(self, segmentation, is_label_map=False, disable_track_bn_stats=False, temperature=2):
        ...
        新的改动，进入细化网络的通道数为1，这样对于非label进行sigmoid，label不做处理
        '''
    ...
    def perturb_latent_code(self, latent_code, decoder_function, label_y=None,
        ...
        if perturb_type == 'dropout':
            ...
            if perturb_type == 'spatial':
                masked_latent_code, mask = mask_latent_code_spatial_wise(latent_code, num_classes=self.num_classes, decoder_function=decoder_function, features=self.features, use_features=True if self.network_type=='TAGNet' else False,
                ...
                masked_latent_code, mask = mask_latent_code_channel_wise(latent_code, num_classes=self.num_classes, decoder_function=decoder_function, features=self.features, use_features=True if self.network_type=='TAGNet' else False,
                                                                         label=label_y, percentile=threshold, random=random_threshold, loss_type=loss_type, if_detach=if_detach, if_soft=if_soft)
            else:
                raise NotImplementedError
        if if_detach:
...
if __name__ == '__main__':
    ...
    solver = AdvancedTripletReconSegmentationModel_2(image_ch=3,
