...
from medseg.models.ebm.TAGEncoder import TAGEncoder
...
class AdvancedTripletReconSegmentationModel(nn.Module):
    ...
    def get_network(self, checkpoint_dir=None):
        ...
        if network_type in ['FCN_16_standard', 'FCN_16_standard_w_o_filter', 'FCN_16_standard_share_code', 'TAGNet']:
            ...
            image_decoder = MyDecoder(input_channel=512 // reduce_factor, up_type='Conv2', output_channel=1,
...
if __name__ == '__main__':
    ...
    solver = AdvancedTripletReconSegmentationModel(image_ch=1,
        network_type='FCN_16_standard', num_classes=2, n_iter=2, use_gpu=True)
    model = solver.model
    images = torch.randn(8, 3, 256, 256).to('cuda')
    pred = solver.evaluate(images, targets_npy=torch.rand([8,256,256]).numpy(), n_iter=2)
