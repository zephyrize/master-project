...
class CorAtt_Unet(nn.Module):
    '''U-net model'''

    def __init__(self, img_ch=1, output_ch=1):
        super(CorAtt_Unet, self).__init__()
        ...
        self.corAtt1 = CoordAtt(filters[0], filters[0])
        self.corAtt2 = CoordAtt(filters[1], filters[1])
        self.corAtt3 = CoordAtt(filters[2], filters[2])
        self.corAtt4 = CoordAtt(filters[3], filters[3])
    ...
    def forward(self, x):
        ...
        de4 = torch.cat((self.corAtt4(res_conv4), de4), dim=1)
        de4 = self.conv4_(de4)

        de3 = self.up_conv4(de4)
        de3 = torch.cat((self.corAtt3(res_conv3), de3), dim=1)
        de3 = self.conv3_(de3)

        de2 = self.up_conv3(de3)
        de2 = torch.cat((self.corAtt2(res_conv2), de2), dim=1)
        de2 = self.conv2_(de2)

        de1 = self.up_conv2(de2)
        de1 = torch.cat((self.corAtt1(res_conv1), de1), dim=1)
        de1 = self.conv1_(de1)
