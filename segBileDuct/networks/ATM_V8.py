...
class AxialBlock(nn.Module):
    ...
    def forward(self, features):
        ...
        '''
        features: type: tensor; shape: (batch, channel, s, h, w)
        '''

        f_AATM = self.AATM(features)

        return f_AATM


class Encoder2(nn.Module):
    def __init__(self, in_ch, out_ch, sample_slices) -> None:
        super(Encoder2, self).__init__()
        
        self.slices = sample_slices
    ...
    def forward(self, x):
        '''
        param: x : shape: (batch * sample_slices, channel, h, w)
        '''
        
        features = self.conv(x)

        n_batch, channel, h, w = features.size()

        batch = n_batch // self.slices
        five_dim_features = features.view(batch, self.slices, channel, h, w).permute(0, 2, 1, 3, 4).contiguous()
        
        return features, five_dim_features
...
class BackBone(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, img_size, sample_slices=3, patch_size=16, embed_dim=384, attention_heads=12, block_nums=[2,2,4,2]) -> None:
        ...
        self.encoder2 = Encoder2(in_ch, out_ch, sample_slices)
    ...
    def forward(self, x):
        ...
        param: x : shape: (batch * sample_slices, channel, h, w)
        '''
        four_dim_features, five_dim_features = self.encoder2(x)
        
        f_AATM = self.axial_block(five_dim_features)

        return four_dim_features, f_AATM
...
def get_Unet_ATM_V8(img_size, dsv=False, sample_slices=3):
...
if __name__ == '__main__':
    ...
    slices = 3
    x = torch.rand([8, slices, 256, 256])

    model = get_Unet_ATM_V8(256, False, slices)
