...
两个分支，第一个conv不再maxpool

backbone提取的特征进入axial transformer block 堆叠2-4次，然后和conv1得到的特征融合，送入conv2

conv1的输入是上一层conv2池化的输出

（在第一版中，conv1的输入是上一次conv1的池化，这个地方应该有点问题）
'''
...
def get_Unet_AATM_V4(img_size, dsv=False):
