...
class BackBone(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, img_size, patch_size=16, embed_dim=384, attention_heads=8, block_nums=[2,2,2,2]) -> None:
    ...
    def forward(self, x1, x2):
        ...
        features_x1 = self.conv1(x1)

        features_x2 = [slice_conv(x2[idx]) for idx, slice_conv in enumerate(self.conv2)]
        
        f_AATM = self.axial_block(features_x2)
        
        assert len(features_x1.shape) == len(f_AATM.shape)
        f_fuse = features_x1 + f_AATM

        return features_x1, features_x2, f_fuse
...
class Unet_AATM(nn.Module):
    ...
    def __init__(self, img_ch, out_ch=1, img_size=256, dsv=False) -> None:
        ...
        # self.dsv1 = nn.Conv2d(filters[1], out_ch, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # encoder
        features1_1, features1_2, f_aatm1 = self.backbone1(x, input)
        res_conv1 = self.conv1(f_aatm1)
        features1_1_pool = self.Maxpool(features1_1)
        features1_2_pool = [self.Maxpool(features1_2[i]) for i in range(slices_num)]
        x2 = self.Maxpool(res_conv1)
        
        features2_1, features2_2, f_aatm2 = self.backbone2(features1_1_pool, features1_2_pool)
        res_conv2 = self.conv2(f_aatm2 + x2)
        features2_1_pool = self.Maxpool(features2_1)
        features2_2_pool = [self.Maxpool(features2_2[i]) for i in range(slices_num)]
        x3 = self.Maxpool(res_conv2)

        features3_1, features3_2, f_aatm3 = self.backbone3(features2_1_pool, features2_2_pool)
        res_conv3 = self.conv3(f_aatm3 + x3)
        features3_1_pool = self.Maxpool(features3_1)
        features3_2_pool = [self.Maxpool(features3_2[i]) for i in range(slices_num)]
        x4 = self.Maxpool(res_conv3) 

        features4_1, features4_2, f_aatm4 = self.backbone4(features3_1_pool, features3_2_pool)
        res_conv4 = self.conv4(f_aatm4 + x4)
        # features4_1_pool = self.Maxpool(features4_1)
        # features4_2_pool = [self.Maxpool(features4_2[i]) for i in range(slices_num)]
...
def get_Unet_AATM(img_size, dsv=False):
