...
from networks.init_weight import *
from networks.utils_layers import MLP, conv_block, up_conv, AxialPositionalEmbedding, DSV, calculate_permutations, sort_and_return_indices
import math
...
class PermuteToFrom(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        q1 = x[:,:,0,...].unsqueeze(2).repeat(1,1,3,1,1)
        q2 = x[:,:,2,...].unsqueeze(2).repeat(1,1,3,1,1)
        
        q1 = q1.permute(*self.permutation).contiguous()
        q2 = q2.permute(*self.permutation).contiguous()
        kv = x.permute(*self.permutation).contiguous()
...
class SelfAttention(nn.Module):
    def __init__(self, dim, heads, dim_heads = None):
        ...
        self.heads = heads
        self.to_q1 = nn.Linear(dim, dim_hidden, bias = False)
        self.to_q2 = nn.Linear(dim, dim_hidden, bias = False)
        self.to_kv = nn.Linear(dim, 2 * dim_hidden, bias = False)
        self.to_out = nn.Linear(dim_hidden, dim)

        self.max_relative_position = 2

        self.relative_position_k = RelativePosition(self.dim_heads, self.max_relative_position)
        self.relative_position_v = RelativePosition(self.dim_heads, self.max_relative_position)

    def forward(self, q1, q2, kv):
        
        print(q1.shape)
        q1, q2, k, v = (self.to_q1(q1), self.to_q2(q2), *self.to_kv(kv).chunk(2, dim=-1))

        b, t, d, h, e = *q1.shape, self.heads, self.dim_heads

        merge_heads = lambda x: x.reshape(b, -1, h, e).transpose(1, 2).reshape(b * h, -1, e)
        
        q1, q2, k, v = map(merge_heads, (q1, q2, k, v))

        dots1 = torch.einsum('bie,bje->bij', q1, k) * (e ** -0.5)
        dots1 = dots1.softmax(dim=-1)

        dots2 = torch.einsum('bie,bje->bij', q2, k) * (e ** -0.5)
        dots2 = dots2.softmax(dim=-1)
...
class SelfAttention(nn.Module):
    def __init__(self, dim, heads, dim_heads = None):
        ...
        self.scale = torch.sqrt(torch.FloatTensor([self.dim_heads])).to(get_device())
    ...
    def forward(self, q1, q2, kv):
        ...
        # q1-k 
        r_q1 = q1.view(b, -1, self.heads, self.dim_heads).permute(0, 2, 1, 3)
        r_k1 = k.view(b, -1, self.heads, self.dim_heads).permute(0, 2, 1, 3)
        attn1 = torch.matmul(r_q1, r_k1.permute(0, 1, 3, 2))

        r_q2 = q1.permute(1, 0, 2).contiguous().view(t, b*self.heads, self.dim_heads)
        r_k2 = self.relative_position_k(t, t)
        attn2 = torch.matmul(r_q2, r_k2.transpose(1, 2)).transpose(0, 1)
        attn2 = attn2.contiguous().view(b, self.heads, t, t)
        attn = (attn1 + attn2) / self.scale

        # print('attn1: ', attn1.device)
        # print('attn2: ', attn2.device)
        # print('scale: ', self.scale.device)
    
        dots1 = self.dropout(torch.softmax(attn, dim = -1))

        # q2-k 
        r_q2 = q2.view(b, -1, self.heads, self.dim_heads).permute(0, 2, 1, 3)
        attn1 = torch.matmul(r_q2, r_k1.permute(0, 1, 3, 2))

        r_q2 = q2.permute(1, 0, 2).contiguous().view(t, b*self.heads, self.dim_heads)
        attn2 = torch.matmul(r_q2, r_k2.transpose(1, 2)).transpose(0, 1)
        attn2 = attn2.contiguous().view(b, self.heads, t, t)
        attn = (attn1 + attn2) / self.scale
        

        dots2 = self.dropout(torch.softmax(attn, dim = -1))

        # sum attention map
        dots = dots1 + dots2 

        r_v1 = v.view(b, -1, self.heads, self.dim_heads).permute(0, 2, 1, 3)
        weight1 = torch.matmul(dots, r_v1)
        r_v2 = self.relative_position_v(t, t)

        weight2 = dots.permute(2, 0, 1, 3).contiguous().view(t, b*self.heads, t)
        weight2 = torch.matmul(weight2, r_v2)
        weight2 = weight2.transpose(0, 1).contiguous().view(b, self.heads, t, self.dim_heads)
...
def get_Unet_AATM_V8(img_size, dsv=False):
