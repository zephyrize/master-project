...
class Attention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        ...
        return x  # return shape: [b, p, p, win*win, head_num, all_head_size/head_num]

    def forward(self, x):
        '''
        param x shape: [b, p, p, win*win, c] 
        '''
        ...
        if self.axial:
            ...
            # print("attention probs shape: ", atten_probs.shape)
...
class WinAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('before win attention shape: ', x.shape)
        b, n, c = x.shape
        ...
        # print('after win attention shape: ', x.shape)
        return x
...
class CSAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('CSAttention input shape: ', x.shape)
        origin_size = x.shape
...
class Stem(nn.Module):
    def __init__(self):
        ...
        # 这个地方的维度应该是原始大小经过三次降采样后的大小： 32 * 32 = 1024
    ...
    def forward(self, x):
        ...
        x = x.flatten(2)  # (B, C, N)  (1, 256, 28*28)
...
class MTUNet(nn.Module):
    ...
    def forward(self, x):
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)
        x, features = self.stem(x)  #(B, N, C) (B, 1024, 256)

        # print('after encoder stem shape', x.shape)
        skips = []
