...
基于第 V4 的改动
backbone的特征提取器增加维度，使用参数scale来指定维度

scale = 1 时 等同于 AAATM—V4
...
class BackBone(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, img_size, patch_size=16, embed_dim=384, attention_heads=12, block_nums=[2,2,4,2], scale=2) -> None:
...
class Unet_AATM(nn.Module):
    ...
    def __init__(self, img_ch, out_ch=1, img_size=256, dsv=False, scale=1) -> None:
        ...
        self.backbone1 = BackBone(in_ch=img_ch, out_ch=filters[0], img_size=img_size, scale=scale) # 1->16->16
        self.encoder1 = Encoder(in_ch=img_ch*3, out_ch=filters[0]) # 3->16->16

        self.backbone2 = BackBone(in_ch=filters[0]*scale, out_ch=filters[1], img_size=img_size//2, scale=scale) # 16->32->32
        self.encoder2 = Encoder(in_ch=filters[0], out_ch=filters[1]) # 16->32->32

        self.backbone3 = BackBone(in_ch=filters[1]*scale, out_ch=filters[2], img_size=img_size//4, scale=scale)
        self.encoder3 = Encoder(in_ch=filters[1], out_ch=filters[2])

        self.backbone4 = BackBone(in_ch=filters[2]*scale, out_ch=filters[3], img_size=img_size//8, scale=scale)
...
def get_Unet_AATM_V6(img_size, dsv=False, scale=1):

    print('use deep supervision: {}'.format(dsv))
    model = Unet_AATM(1, 1, img_size=img_size, dsv=dsv, scale=scale)
    return model
...
if __name__ == '__main__':
    ...
    model = Unet_AATM(1, 1, 256, False, 4)
