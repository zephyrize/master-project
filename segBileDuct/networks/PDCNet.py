...
from networks.pidi.config import personal_config
from networks.pidi.ops import Conv2d
from networks.pidi.pidinet import PDCBlock, MapReduce, CDCM, CSAM
class EdgeBlock(nn.Module):

    def __init__(self, in_ch, plt=False): # plt: if interpolate
        super(EdgeBlock, self).__init__()

        self.plt = plt
        pdcs = personal_config('carv4')
        
        block_class = PDCBlock

        self.edge = nn.Sequential(
            Conv2d(pdcs[0], in_ch, in_ch*2, kernel_size=3, padding=1),
            block_class(pdcs[1], in_ch*2, in_ch*2),
            block_class(pdcs[2], in_ch*2, in_ch*2),
            block_class(pdcs[3], in_ch*2, in_ch*2),
            CDCM(in_ch*2, 24),
            CSAM(24),
            MapReduce(24)
        )
    ...
    def forward(self, x) :
        ...
        if self.plt is True:
            edge = F.interpolate(edge, (256, 256), mode='bilinear', align_corners=False)
...
class Unet_AATM(nn.Module):
    ...
    def __init__(self, img_ch, out_ch=1, img_size=256, dsv=False, sample_slices=3) -> None:
        ...
        self.get_edge1 = EdgeBlock(filters[0])
        self.get_edge2 = EdgeBlock(filters[1], plt=True)
        ...
        self.edge_conv = nn.Conv2d(2, 1, kernel_size=1)
        self.sigmoid = nn.Sigmoid()

        self.apply(self.weight)

        nn.init.constant_(self.edge_conv.weight, 0.25)
        nn.init.constant_(self.edge_conv.bias, 0)
    ...
    def forward(self, x):
        ...
        edge1 = self.get_edge1(res_encoder1)
        edge2 = self.get_edge2(res_encoder2)

        edge = self.edge_conv(torch.cat([edge1, edge2], dim=1))
        ...
        if self.dsv is True:
            ...
            return [output, [slice_prop_1, slice_prop_2, slice_prop_3, slice_prop_4], edge]



def get_Unet_PDCNet(img_size, dsv=False, sample_slices=3):

    print('use deep supervision: {}'.format(dsv))
    model = Unet_AATM(1, 1, img_size=img_size, dsv=dsv, sample_slices=sample_slices)
    return model
...
if __name__ == '__main__':
    ...
    model = get_Unet_PDCNet(256, False, 3)
