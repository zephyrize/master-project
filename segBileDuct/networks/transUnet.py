...
class Encoder(nn.Module):
    def __init__(self, img_dim, in_channels, out_channels, head_num, mlp_dim, block_num, patch_dim):
        ...
        self.vit_img_dim = img_dim // patch_dim # 16
    ...
    def forward(self, x):
        ...
        print('after first conv layer shape: ', x1.shape)
        x2 = self.encoder1(x1)
        x3 = self.encoder2(x2)
        x = self.encoder3(x3)
        print('after three encoders conv layer shape: ', x.shape)
...
if __name__ == '__main__':
    ...
    total_params = sum([param.nelement() for param in transunet.parameters()])
    
    stat(transunet, (3,256,256))
