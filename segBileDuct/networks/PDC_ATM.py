...
from networks.pidi.pidinet import pidinet_small, MapReduce
...
class EdgeBlock(nn.Module):
    ...
    def __init__(self, in_ch):
        ...
        pidi_model = pidinet_small()


        self.edge = nn.Sequential(
            nn.Conv2d(in_ch, 30, kernel_size=3, stride=1, padding=1, bias=False),
            pidi_model.block1_1,
            pidi_model.block1_2,
            pidi_model.block1_3,
            pidi_model.dilations[0],
            pidi_model.attentions[0]
        )
        
        self.conv_reduce = MapReduce(12)

    def forward(self, x) :
        
        x = self.edge(x)

        edge = self.conv_reduce(x)
...
class Unet_AATM(nn.Module):
    ...
    def __init__(self, img_ch, out_ch=1, img_size=256, dsv=False, sample_slices=3) -> None:
        ...
        self.get_edge = EdgeBlock(filters[0])
    ...
    def forward(self, x):
        ...
        edge = self.get_edge(res_encoder1)
...
def get_Unet_PDC_ATM(img_size, dsv=False, sample_slices=3):
...
if __name__ == '__main__':
    ...
    model = get_Unet_PDC_ATM(256, False, 3)
